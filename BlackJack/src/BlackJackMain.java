...
public class BlackJackMain {
	...
	public static void main(String[] args) {
		...
		/**
		 * Developed 2017 by: Moa Gardsäter Sara Svensson Aleksander Frostelen Jaimen
		 * Lavalle Steffo Dimfelt
		 */
		...
		for (int handIndex = 0; handIndex < player.hand.size(); handIndex++) {
			...
			if (deck.totalHandValue(dealer, 0) <= 21) {
				if (deck.totalHandValue(player, handIndex) <= 21) {
					if (deck.totalHandValue(player, handIndex) == deck.totalHandValue(dealer, 0)) {
						...
						if (deck.totalHandValue(player, handIndex) >= 17) {
							System.out.println("Du fick " + deck.totalHandValue(player, handIndex)
									+ " och får behålla din insats.");
				...
				if (deck.totalHandValue(player, handIndex) <= 21) {
					...
					if (deck.totalHandValue(player, handIndex) == 21) {
						System.out.println(
								"Du fick " + deck.totalHandValue(player, handIndex) + " och får tillbaka 3x insatsen.");
						for (int bettingElem = 0; bettingElem < betting.getPlayerBetSize(); bettingElem++) {
							betting.bettingPayBack(bettingElem, 2);
						}
					} else {
						System.out.println(
								"Du fick " + deck.totalHandValue(player, handIndex) + " och får tillbaka 2x insatsen.");
						for (int bettingElem = 0; bettingElem < betting.getPlayerBetSize(); bettingElem++) {
							betting.bettingPayBack(bettingElem, 1);
						}
					}
	...
	public void splitHands() {
		...
		do {
				...
				if (player.hand.get(handIndex).get(0).equals(player.hand.get(handIndex).get(1))) {
					if (betting.getPlayerChips() >= betting.getBettingValue()) {
						...
						if (splitCards == true) {
							...
							player.hand.get(handIndex + 1).add(new Integer(tempElement1));// Kopierar värde från gammal
																							// array till ny
							player.hand.get(handIndex).clear();// Nollställer Row
							player.hand.get(handIndex).add(new Integer(tempElement0));// Kopierar tillbaka värdet till
																						// gammal array.

							player.handCol.get(handIndex + 1).add(new String(tempElementCol1));// Kopierar värde från
																								// gammal array till ny
							player.handCol.get(handIndex).clear();// Nollställer Row
							player.handCol.get(handIndex).add(new String(tempElementCol0));// Kopierar tillbaka värdet
																							// till gammal array.
							...
							for (int splitRow = 0; splitRow < player.hand.size(); splitRow++) {
								...
								System.out.println("På Hand " + tempSplitRow + " fick du: "
										+ deck.showOneCard(player, splitRow, 1));
			...
			if (checkedAllHands == player.hand.size()) {
				splitHands = true;
			}
