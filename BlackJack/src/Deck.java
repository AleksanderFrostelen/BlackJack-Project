...
public class Deck {
	...
	void shuffle() {
		...
		//	for (int suit = 0; LinkedElem < shoeCol.size(); LinkedElem++) {
			
				reload.add(shoeCol.get(LinkedElem));
			}
		}
	...
	public void aceDecision(Player playerObj) 
	{
		...
		{
			if(playerObj.hand.get(i).equals(1) || (playerObj.hand.get(i).equals(11)
			{
				System.out.println("Ska Esset vara 1 eller 11? Ange '1' eller '11', tryck sedan på enter.");
				ace = input.nextInt();
				if(ace == 1) 
				{
					playerObj.hand.set(1);
				}
				else
				{
					playerObj.hand.set(11);
				}
			}
		}
	}
